...
export class MinHeap<T> {
    ...
    public peek(): HeapItem<T> { return this.arr[0]; }
     ...
     */
    public popPush(key: number, value: T): HeapItem<T> {
        if (this.arr.length === 0) {
            throw new Error('pop() called on empty binary heap');
        }
        const item = this.arr[0];
    ...
    public items(): Array<HeapItem<T>> { return this.arr; }
...
export class KMin<T> {
    ...
    public add(key: number, value: T) {
        ...
        const largest = this.maxHeap.peek();
    ...
    public getMinKItems(): T[] {
        const items = this.maxHeap.items();
        items.sort((a, b) => b.key - a.key);
        return items.map((a) => a.value);
    }

    /** Returns the size of the list. */
    public getSize(): number { return this.maxHeap.size(); }

    /** Returns the largest key in the list. */
    public getLargestKey(): any {
