...
export class SPTree {
    ...
    constructor(data: Point[]) {
        ...
        const min: Point = new Array(this.dim);
        fillArray(min, Number.POSITIVE_INFINITY);
        const max: Point = new Array(this.dim);
        ...
        this.root = {box: {center, halfDim}, point: data[0]};
     ...
     * corner.
     */
    public visit(accessor: (node: SPNode, lowPoint?: number[] | undefined, highPoint?: number[] | undefined) => boolean, noBox = false) {
        this.visitNode(this.root, accessor, noBox);
    }

    private visitNode(node: SPNode, accessor: (node: SPNode, lowPoint?: Point, highPoint?: Point) => boolean, noBox: boolean) {
