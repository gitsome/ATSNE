 ...
 */
export function centroid<T>(dataPoints: T[], accessor?: (a: T) => Vector): Vector|null {
  if (dataPoints.length === 0) {
    return null;
  }
  if (accessor == null) {
    accessor = (a: T) => a as any;
  }
  assert(dataPoints.length >= 0, '`vectors` must be of length >= 1');

  const centroidVar = new Float32Array(accessor(dataPoints[0]).length);
  for (let i = 0; i < dataPoints.length; ++i) {
    const dataPoint = dataPoints[i];
    const vector = accessor(dataPoint);
    for (let j = 0; j < centroidVar.length; ++j) {
        centroidVar[j] += vector[j];
    }
  }
  for (let j = 0; j < centroidVar.length; ++j) {
    centroidVar[j] /= dataPoints.length;
  }
  return centroidVar;
}
 ...
 */
export function toTypedArray<T>(dataPoints: T[], accessor: (dataPoint: T) => Float32Array): Float32Array {
  const N = dataPoints.length;
  const dim = accessor(dataPoints[0]).length;
  const result = new Float32Array(N * dim);
  for (let i = 0; i < N; ++i) {
    const vector = accessor(dataPoints[i]);
    for (let d = 0; d < dim; ++d) {
...
export function transposeTypedArray(
  ...
  const result = new Float32Array(r * c);
  for (let i = 0; i < r; ++i) {
    for (let j = 0; j < c; ++j) {
