...
export function vector3DToScreenCoords(cam: THREE.Camera, w: number, h: number, v: THREE.Vector3): Point2D {
  const dpr = window.devicePixelRatio;
  const pv = new THREE.Vector3().copy(v).project(cam);

  // The screen-space origin is at the middle of the screen, with +y up.
  const coords: Point2D = [((pv.x + 1) / 2 * w) * dpr, -((pv.y - 1) / 2 * h) * dpr];
  return coords;
}
 ...
 */
export function createTexture(image: HTMLImageElement|HTMLCanvasElement): THREE.Texture {
...
export function getSearchPredicate(
  ...
  if (inRegexMode) {
    ...
    predicate = (p) => {
      const label = p.metadata[fieldName].toString().toLowerCase();
 ...
 */
export function runAsyncTask<T>(message: string, task: () => T, msgId: string = ''): Promise<T> {
  const autoClear = (msgId === '');
...
export function getURLParams(url: string): {[key: string]: string} {
  ...
  for (let i = 0; i < queryEntries.length; i++) {
    const queryEntryComponents = queryEntries[i].split('=');
